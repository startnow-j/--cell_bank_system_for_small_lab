...
if [ -d "db" ]; then
    echo "  - 复制 db"
    cp -r db "$BUILD_DIR/next-service-dist/"
fi
...
if [ -d "docs" ]; then
    echo "  - 复制 docs"
    cp -r docs "$BUILD_DIR/next-service-dist/"
fi
...
# 安装 standalone 缺失的依赖
echo "📦 安装 standalone 缺失依赖..."
cd "$BUILD_DIR/next-service-dist"
bun add bcryptjs xlsx date-fns recharts zustand @tanstack/react-query 2>&1 || echo "⚠️  部分依赖安装失败，继续..."
